...
This document analyzes the spatiotemporal dynamics of viral communities (vOTUs) in Arbor Acres chickens across different gut segments (foregut vs. hindgut) and developmental stages (1-56 days). Non-metric multidimensional scaling (NMDS) ordination is used to visualize community structure and PERMANOVA is applied to test for significant differences.
...
params <- list(name = "12-days-gut-AA")
here::i_am(paste0(params$name, ".qmd"), uuid = "d03d5076-6f65-455d-86b1-6a7cb4e4b51e")
...
Filter samples from Arbor Acres chickens (Beijing) without feed additives, and classify gut segments into foregut and hindgut.

```{r}
#| label: sample-metadata

map_guts_AA <- metadata417359 %>%
  dplyr::filter(feed_addictive_.growth_promoter == "None") %>%
  dplyr::filter(geo_loc_name == "China:Beijing") %>%
  dplyr::select(Run, Sample.Name, isolation_source, Host_Age)

map_guts_AA2 <- map_guts_AA %>%
  dplyr::mutate(
...
cat("Total samples:", nrow(map_guts_AA2), "\n")
cat("Foregut samples:", sum(map_guts_AA2$isolation_source == "Foregut"), "\n")
cat("Hindgut samples:", sum(map_guts_AA2$isolation_source == "Hindgut"), "\n")
```
...
guts_contig_annotation <- chicken_contig_annotation %>%
  ...
  dplyr::filter(study_id %in% map_guts_AA2$Run)
...
ho_raw <- as.data.frame(ho_raw) %>%
  ...
  dplyr::filter(rownames %in% map_guts_AA2$Run) %>%
...
scores$Host_Age <- factor(scores$Host_Age, levels = c("1-day", "7-day", "14-day", "28-day", "42-day"))
...
plot_NMDS1 <- ggplot2::ggplot(scores, ggplot2::aes(x = NMDS1, y = NMDS2, color = Host_Age, shape = isolation_source)) + 
  ...
  ggplot2::scale_color_manual(values = c("#F8766D", "orange", "#E76BF3", "#00BF7D", "#00B0F6", "red", "black")) + 
  ggplot2::theme(
...
plot_NMDS2 <- ggplot2::ggplot(scores, ggplot2::aes(x = NMDS1, y = NMDS2, color = Host_Age, shape = isolation_source)) + 
  ...
  ggplot2::scale_shape_manual(values = c(16, 2, 15, 17)) + 
  ggplot2::scale_color_manual(values = c("#F8766D", "orange", "#E76BF3", "#00BF7D", "#00B0F6")) + 
  ggplot2::theme(
    axis.text = ggplot2::element_text(color = "black"),
    axis.title = ggplot2::element_text(color = "black", face = "bold"),
    plot.title = ggplot2::element_text(hjust = 0.5)
