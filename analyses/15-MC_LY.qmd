...
params <- list(name = "15-MC_LY")
here::i_am(paste0(params$name, ".qmd"), uuid = "85fc4248-450b-4554-9759-f94cffef597a")
...
Filter 56-day-old Lingnan Yellow chickens from Hunan across different feed additive treatments.

```{r}
#| label: sample-metadata

mappingMCE_LY <- metadata417359 %>%
  dplyr::filter(
    geo_loc_name == "China:Hunan",
    Host_Age == "56-day"
  ) %>%
  dplyr::mutate(
...
# The abundance of the MCE-H group was not detected and is therefore excluded.
mappingMCE_for <- mappingMCE_LY %>%
  dplyr::filter(Sample.Name != "MCE-H", isolation_source == "Foregut")

mappingMCE_for2 <- mappingMCE_for %>%
  dplyr::select(Run, Sample.Name)

mappingMCE_hid <- mappingMCE_LY %>%
  dplyr::filter(Sample.Name != "MCE-H", isolation_source == "Hindgut")
...
cat("Treatment groups:", paste(unique(mappingMCE_LY$Sample.Name), collapse = ", "), "\n")
```
...
MCELY_contig_annotation <- chicken_contig_annotation %>%
  dplyr::filter(study_id %in% mappingMCE_for$Run)

MCELY_h_ctg <- MCELY_contig_annotation %>%
  dplyr::filter(contig_id == contig_repid) %>%
  dplyr::filter(checkv_quality_score < 4) %>%
  dplyr::select(contig_repid, vc_id, vOTU_id)

MCELY_abundance_tmm <- abundance_contigs_tmm %>%
  dplyr::filter(Contig %in% MCELY_h_ctg$contig_repid) %>%
  dplyr::select(Contig, dplyr::one_of(mappingMCE_for$Run))

votu_rawt <- MCELY_abundance_tmm

map_votu <- chicken_contig_annotation %>%
  dplyr::filter(contig_repid %in% MCELY_h_ctg$contig_repid) %>%
...
plot_NMDS1 <- ggplot2::ggplot(scores, ggplot2::aes(x = NMDS1, y = NMDS2, color = Sample.Name, shape = isolation_source)) + 
  ...
  ggplot2::theme_bw() + 
  ggplot2::labs(
    title = "NMDS Plot (Sample.Name vs. isolation_source)",
    x = "NMDS Axis 1",
    y = "NMDS Axis 2"
  ) + 
...
diverso_long$group <- factor(diverso_long$group, levels = c("Blank", "CTC", "MCE-L", "MCE-M"))

groups <- c("Blank", "CTC", "MCE-L", "MCE-M")
...
ggplot2::ggsave(path_target("diver_for_votu.pdf"), plot = diver_plot_votu, width = 10, height = 5)
ggplot2::ggsave(path_target("diver_for_votu.png"), plot = diver_plot_votu, width = 10, height = 5)
```
...
ctg_h_LY <- chicken_contig_annotation %>%
  dplyr::filter(contig_id == contig_repid) %>%
  dplyr::filter(checkv_quality_score < 4) %>%
  dplyr::filter(study_id %in% mappingMCE_for$Run) %>%
  dplyr::select(contig_id, study_id)

Host_LY <- chicken_host %>%
  dplyr::filter(contig_id %in% ctg_h_LY$contig_id) %>%
...
vOTU_h_LY <- chicken_contig_annotation %>%
  dplyr::filter(contig_id == contig_repid) %>%
  dplyr::filter(checkv_quality_score < 4) %>%
  dplyr::filter(study_id %in% mappingMCE_for$Run) %>%
  dplyr::select(contig_id, study_id, vOTU_id)

LY_abundance_count <- abundance_contigs_count %>%
  dplyr::filter(Contig %in% vOTU_h_LY$contig_id) %>%
  dplyr::select(Contig, dplyr::one_of(mappingMCE_for$Run))

votu_raw_count <- LY_abundance_count

map_votu <- vOTU_h_LY %>%
  dplyr::select(contig_id, vOTU_id)

votu_table_count <- merge(votu_raw_count, map_votu, by.x = "Contig", by.y = "contig_id") %>%
  dplyr::group_by(vOTU_id) %>%
  dplyr::summarize(dplyr::across(-c(Contig), sum), .groups = "drop") %>%
  as.data.frame()

Host_LY_vOTU <- merge(Host_LY, map_votu, by = "contig_id")

abun_Host_LY_vOTU_count <- merge(Host_LY_vOTU, votu_table_count, by = "vOTU_id") %>%
  dplyr::select(Host.taxonomy, dplyr::one_of(mappingMCE_for$Run)) %>%
  unique()

abun_Host_LY_count <- abun_Host_LY_vOTU_count %>%
...
input_abundance <- abun_Host_LY_count
...
tax <- abun_Host_LY_count$Host.taxonomy
tax <- as.matrix(tax)
rownames(tax) <- abun_Host_LY_count$Host.taxonomy
...
res_trend <- res %>%
  dplyr::filter(p_groupCTC < 0.05 | `p_groupMCE-L` < 0.05 | `p_groupMCE-M` < 0.05)
```
...
value_data <- df_fig_group[, c("valueCTC", "valueMCE_L", "valueMCE_M")] %>%
  dplyr::mutate_all(~ round(., 2))

p_data <- df_fig_group[, c("p_groupCTC", "p_groupMCE_L", "p_groupMCE_M")]

rownames(value_data) <- df_fig_group$taxon
colnames(value_data) <- c("CTC", "MCE-L", "MCE-M")
rownames(p_data) <- df_fig_group$taxon
colnames(p_data) <- c("CTC", "MCE-L", "MCE-M")
...
MCELY_abundance_count <- abundance_contigs_count %>%
  dplyr::filter(Contig %in% MCELY_h_ctg$contig_repid) %>%
  dplyr::select(Contig, dplyr::one_of(mappingMCE_for2$Run))

ctg_abundance <- MCELY_abundance_count
```
...
groups_to_compare <- c("CTC", "MCE-L", "MCE-M")
results_wilcox <- list()

for (gene in colnames(data)[2:7]) {
...
ggplot2::ggsave(path_target("pheatmap_AMG_for.png"), plot = pheatmap_AMG_for, width = 8, height = 4)
ggplot2::ggsave(path_target("pheatmap_AMG_for.pdf"), plot = pheatmap_AMG_for, width = 8, height = 4)
```
...
MCELY_contig_annotation_hid <- chicken_contig_annotation %>%
  dplyr::filter(study_id %in% mappingMCE_hid$Run)

MCELY_h_ctg_hid <- MCELY_contig_annotation_hid %>%
  dplyr::filter(contig_id == contig_repid) %>%
  dplyr::filter(checkv_quality_score < 4) %>%
  dplyr::select(contig_repid, vc_id, vOTU_id)

MCELY_abundance_tmm_hid <- abundance_contigs_tmm %>%
  dplyr::filter(Contig %in% MCELY_h_ctg_hid$contig_repid) %>%
  dplyr::select(Contig, dplyr::one_of(mappingMCE_hid$Run))

votu_rawt_hid <- MCELY_abundance_tmm_hid

map_votu_hid <- chicken_contig_annotation %>%
  dplyr::filter(contig_repid %in% MCELY_h_ctg_hid$contig_repid) %>%
  dplyr::select(contig_repid, vOTU_id)
...
scores_hid$Sample.Name <- factor(scores_hid$Sample.Name, levels = c("Blank", "CTC", "MCE-L", "MCE-M"))

plot_NMDS2 <- ggplot2::ggplot(scores_hid, ggplot2::aes(x = NMDS1, y = NMDS2, color = Sample.Name, shape = isolation_source)) + 
  ggplot2::geom_point(size = 4) + 
  ggplot2::theme_bw() + 
  ggplot2::labs(
    title = "NMDS Plot (Sample.Name vs. isolation_source)",
  ...
  ggplot2::scale_color_brewer(palette = "Set1") + 
  ggplot2::scale_shape_manual(values = c(2, 16, 15, 2, 3, 1)) + 
  ggplot2::scale_color_manual(values = c("#F5A800", "#FF4EA0", "#5CDB5C", "#43B343", "#006400"))
...
ggplot2::ggsave(path_target("plot_NMDS_hid.pdf"), plot = plot_NMDS2, width = 8, height = 5)
ggplot2::ggsave(path_target("plot_NMDS_hid.png"), plot = plot_NMDS2, width = 8, height = 5)
```
...
divero_hid <- resulto_hid %>%
  dplyr::rename(group = Sample.Name)

divero_long_hid <- divero_hid %>%
  tidyr::pivot_longer(
    cols = c(Shannon, Chao1, ACE, pielou, Simpson, Richness),
    names_to = "name",
    values_to = "value"
...
divero_long_hid$group <- factor(divero_long_hid$group, levels = c("Blank", "CTC", "MCE-L", "MCE-M"))

groups_hid <- c("Blank", "CTC", "MCE-L", "MCE-M")
my_comparisons1_hid <- combn(groups_hid, 2, simplify = FALSE)

diver_plot_votu2 <- ggplot2::ggplot(divero_long_hid, ggplot2::aes(y = value, x = group, group = group, color = group)) + 
  ggplot2::geom_boxplot(fill = "white", outlier.shape = NA, lwd = 0.8) + 
  ggplot2::theme(
    axis.line = ggplot2::element_line(linewidth = 1, colour = "black", linetype = 1),
    axis.text.x = ggplot2::element_text(
...
print(diver_plot_votu2)

ggplot2::ggsave(path_target("diver_plot_votu_hid.pdf"), plot = diver_plot_votu2, width = 10, height = 5)
ggplot2::ggsave(path_target("diver_plot_votu_hid.png"), plot = diver_plot_votu2, width = 10, height = 5)
```
...
ctg_h_LY_hid <- chicken_contig_annotation %>%
  dplyr::filter(contig_id == contig_repid) %>%
  dplyr::filter(checkv_quality_score < 4) %>%
  dplyr::filter(study_id %in% mappingMCE_hid$Run) %>%
  dplyr::select(contig_id, study_id)

Host_LY_hid <- chicken_host %>%
  dplyr::filter(contig_id %in% ctg_h_LY_hid$contig_id) %>%
  dplyr::select(contig_id, Host.taxonomy) %>%
  dplyr::mutate(Host.taxonomy = gsub("^[^;]*;[^;]*;[^;]*;[^;]*;[^;]*;", "", Host.taxonomy)) %>%
  dplyr::mutate(Host.taxonomy = gsub(";.*", "", Host.taxonomy)) %>%
  dplyr::mutate(Host.taxonomy = ifelse(Host.taxonomy == "", "Unknown", Host.taxonomy)) %>%
...
vOTU_h_LY_hid <- chicken_contig_annotation %>%
  dplyr::filter(contig_id == contig_repid) %>%
  dplyr::filter(checkv_quality_score < 4) %>%
  dplyr::filter(study_id %in% mappingMCE_hid$Run) %>%
  dplyr::select(contig_id, study_id, vOTU_id)

LY_abundance_count_hid <- abundance_contigs_count %>%
  dplyr::filter(Contig %in% vOTU_h_LY_hid$contig_id) %>%
  dplyr::select(Contig, dplyr::one_of(mappingMCE_hid$Run))

votu_raw_count_hid <- LY_abundance_count_hid

map_votu_hid <- vOTU_h_LY_hid %>%
  dplyr::select(contig_id, vOTU_id)
...
Host_LY_vOTU_hid <- merge(Host_LY_hid, map_votu_hid, by = "contig_id")

abun_Host_LY_vOTU_count_hid <- merge(Host_LY_vOTU_hid, votu_table_count_hid, by = "vOTU_id") %>%
  dplyr::select(Host.taxonomy, dplyr::one_of(mappingMCE_hid$Run)) %>%
  unique()

abun_Host_LY_count_hid <- abun_Host_LY_vOTU_count_hid %>%
  dplyr::group_by(Host.taxonomy) %>%
  dplyr::summarise(dplyr::across(where(is.numeric), sum, na.rm = TRUE), .groups = "drop")
```
...
input_abundance_hid <- abun_Host_LY_count_hid
...
tax_hid <- abun_Host_LY_count_hid$Host.taxonomy
tax_hid <- as.matrix(tax_hid)
rownames(tax_hid) <- abun_Host_LY_count_hid$Host.taxonomy
...
value_data_hid <- df_fig_group_hid[, c("valueCTC", "valueMCE_L", "valueMCE_M")] %>%
  dplyr::mutate_all(~ round(., 2))

p_data_hid <- df_fig_group_hid[, c("p_groupCTC", "p_groupMCE_L", "p_groupMCE_M")]

rownames(value_data_hid) <- df_fig_group_hid$taxon
colnames(value_data_hid) <- c("CTC", "MCE-L", "MCE-M")
rownames(p_data_hid) <- df_fig_group_hid$taxon
colnames(p_data_hid) <- c("CTC", "MCE-L", "MCE-M")
...
pheatmap_Genus_hid <- pheatmap::pheatmap(
  mat = value_data_hid,
  color = grDevices::colorRampPalette(c("#0000FF", "#FFFFFF", "#FF0000"))(100),
  breaks = seq(-4.5, 4.5, length.out = 101),
  show_rownames = TRUE,
  show_colnames = TRUE,
  cluster_rows = FALSE,
...
MCELY_abundance_count_hid <- abundance_contigs_count %>%
  dplyr::filter(Contig %in% MCELY_h_ctg_hid$contig_repid) %>%
  dplyr::select(Contig, dplyr::one_of(mappingMCE_hid2$Run))

ctg_abundance_hid <- MCELY_abundance_count_hid
```
...
for (gene in colnames(data_hid)[2:5]) {
  for (grp in groups_to_compare) {
...
ggplot2::ggsave(path_target("pheatmap_AMG_hid.png"), plot = pheatmap_AMG_hid, width = 5, height = 3)
ggplot2::ggsave(path_target("pheatmap_AMG_hid.pdf"), plot = pheatmap_AMG_hid, width = 5, height = 3)
```
